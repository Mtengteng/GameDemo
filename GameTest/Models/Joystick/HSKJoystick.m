...
- (instancetype)initWithAttackButtonImageNamed:(NSString *)backName size:(CGSize)size
{
    if (self = [super init]) {
        ...
        SKSpriteNode *backdropNode = [SKSpriteNode spriteNodeWithImageNamed:backName];
        backdropNode.size = size;
        [self addChild:backdropNode];
        
        CADisplayLink *displayLink = [CADisplayLink displayLinkWithTarget:self selector:@selector(attackAction)];
//        [displayLink addToRunLoop:[NSRunLoop currentRunLoop] forMode:NSRunLoopCommonModes];
    }
    return self;
}
